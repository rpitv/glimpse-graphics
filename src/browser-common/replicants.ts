...
export async function loadReplicants() {
	return {
		...
		gameSettings: {
			...
			style: await replicant<'espn' | 'rpitv-modern' | 'rpitv-classic' | 'rpitv-style7' | 'rpitv-football'>('style', 'glimpse-graphics.game-settings.style', {defaultValue: 'rpitv-modern'}),
			clock: {
