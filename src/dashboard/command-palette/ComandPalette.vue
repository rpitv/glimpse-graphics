<template>
	<div>
		...
		<textarea
			class="palette"
			ref="palette"
			:placeholder="paletteText"
			@keydown="paletteKeyPressed" />
...
import { NButton, NCollapse, NCollapseItem, NTable } from "naive-ui";
...
function areClockCommandsEnabled(): boolean {
	return (
		...
		!replicants.sync.values.clock.value
	);
}
...
const commands: Record<string, Command> = {
	" ": {
		...
		fn() {
			replicants.scoreboard.visible.value =
				!replicants.scoreboard.visible.value;
		},
		enabled() {
			return true;
	...
	k: {
		...
		fn() {
			replicants.scoreboard.clock.isRunning.value =
				!replicants.scoreboard.clock.isRunning.value;
		},
...
const paletteText = ref<string>(
	"Command palette not in focus. Click here to use command palette."
);
...
watchEffect(() => {
	if (palette.value !== null) {
		palette.value.addEventListener("focusin", () => {
			paletteText.value =
				"Command palette in focus. Ready to receive commands.";
