<template>
	<div>
		<n-grid :cols="2">
			<n-grid-item>
				<n-checkbox class="ml-10" v-model:checked="isTeamEnabled"
				>Enabled
				</n-checkbox
				>
			</n-grid-item>
			<n-grid-item>
				<n-checkbox class="ml-10" v-model:checked="syncScore"
				>Sync Score
				</n-checkbox
				>
			</n-grid-item>
			<n-grid-item>
				<n-checkbox class="ml-10" v-model:checked="syncName"
				>Sync Name
				</n-checkbox
				>
			</n-grid-item>
			<n-grid-item>
				<n-checkbox class="ml-10" v-model:checked="syncAbbreviation"
				>Sync Abbr.
				</n-checkbox
				>
			</n-grid-item>
		...
		<div v-if="isTeamEnabled">
			...
			<div class="mt-10">
				...
				<n-input
					:id="teamAbbrId"
					:disabled="syncAbbreviation"
					v-model:value="teamAbbr" />
			</div>
			...
			<div class="mt-10">
				...
				<n-input-group :id="teamColorsId">
					<n-color-picker
						...
						:show-preview="true"
						:modes="['hex']"
						v-model:value="teamPrimaryColor" />
					<n-color-picker
						:show-alpha="false"
						:show-preview="true"
						:modes="['hex']"
						v-model:value="teamSecondaryColor" />
				</n-input-group>
			</div>

			<div class="mt-10">
				<label :for="teamLogoId"
				>Team Logo <small>(Only input trusted URLs.)</small></label
				>
			...
			<div class="team-logo-container">
				<img
					class="team-logo mt-10"
					v-if="teamLogo?.length > 0"
					:src="teamLogo"
					alt="Team Logo" />
			</div>
		</div>
...
import { v4 } from "uuid";
import { NCheckbox, NColorPicker, NGrid, NGridItem, NInput, NInputGroup } from "naive-ui";
