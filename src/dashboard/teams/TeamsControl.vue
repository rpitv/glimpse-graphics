<template>
	<div>
		<n-grid
			:cols="isAwayTeamEnabled && isHomeTeamEnabled ? 2 : 1"
			:x-gap="20">
			<n-grid-item v-if="isAwayTeamEnabled">
				<TeamControl
					v-model:score="awayTeamScore"
					:synced="awayTeamScoreSynced" />
			</n-grid-item>
			<n-grid-item v-if="isHomeTeamEnabled">
				<TeamControl
					v-model:score="homeTeamScore"
					:synced="homeTeamScoreSynced" />
			</n-grid-item>
		</n-grid>
	</div>
</template>
