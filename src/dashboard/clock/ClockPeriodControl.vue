	...
	<div>
		<div class="clock-section" v-if="isClockEnabled && !isClockSynced">
			...
			<div class="clock-offset-buttons">
				<n-button type="info" @click="setClock(currentTime - 5000)"
				>-5s
				</n-button
				>
				<n-button type="info" @click="setClock(currentTime - 1000)"
				>-1s
				</n-button
				>
				<n-button type="info" @click="setClock(currentTime - 100)"
				>-0.1s
				</n-button
				>
				<n-button type="info" @click="setClock(currentTime + 100)"
		...
		<div
			...
			v-if="arePeriodsEnabled && !arePeriodsSynced">
			...
			<div class="period-slider">
				<n-button
					...
					@click="
						currentPeriod =
							currentPeriod > 1 ? currentPeriod - 1 : 1
					">
					-1
				</n-button>
				<n-slider
		...
		<n-alert
			v-if="
				!arePeriodsSynced &&
				arePeriodsEnabled &&
				(overtimeCount > 3 || isOvertimeInfinite)
			"
			type="info"
			class="control-alert">
			The period slider is only capable of supporting up to 3 overtime
			periods.
		</n-alert>

		<n-input-group class="clock-period-text-input">
			<n-input
				v-if="isClockEnabled && !isClockSynced"
				placeholder="00:00.0"
				type="text"
		...
		<n-alert v-if="syncedFeatures" type="info" class="control-alert">
			{{ syncedFeatures }} cannot be manually controlled while Daktronics
			RTD sync is enabled.
		</n-alert>
	</div>
</template>
