	...
	<div>
		...
		<div class="mt-10">
			...
			<n-select
				:id="syncPathId"
				clearable
				v-model:value="replicants.sync.selectedPort.value"
		...
		<div class="mt-10">
			...
			<n-select
				:id="sportId"
				v-model:value="replicants.sync.selectedSport.value"
				:options="availableSportsOptions" />
		</div>

		<div class="mt-10">
			<n-checkbox
				v-model:checked="showAdvancedSettings"
				@click="confirmShowAdvanced"
			>Advanced Settings
			</n-checkbox
			>
		</div>
		...
		<div
			...
			<n-alert title="Packet Information" type="info">
				<p>
					If the data you input is less than the expected length, it
					will be padded with spaces according to its justification.
					If the data you input is greater than the expected length,
					it will all be sent, however the backend will likely trim it
				...
				<div v-if="selectedMockPacketInfo">
					...
					<ul>
						<li>
							Justification:
							{{
								selectedMockPacketInfo.justification === "L"
									? "Left"
									: "Right"
							}}
						</li>
						<li>
							Expected Length:
							{{ selectedMockPacketInfo.length }} bytes
						</li>
					</ul>
					...
					<ul>
						<li>
							ASCII: "<kbd>{{
								mockPacketPaddedDataInputAsciiDisplay
							}}</kbd
						>"
						</li>
						<li>
							Hexadecimal:
							<kbd>{{ mockPacketPaddedDataInputHexDisplay }}</kbd>
						</li>
					</ul>
				</div>
			</n-alert>
			<div class="mt-10">
				...
				<n-select
					:id="mockPacketId"
					v-model:value="mockPacketSelection"
					:options="mockPacketOptions" />
			</div>
			<div class="mt-10">
				<label :for="mockPacketDataId">Data</label>
				<n-input
					:id="mockPacketDataId"
					v-model:value="mockPacketDataInput" />
			</div>
			<n-button class="mt-10" @click="submitMockPacket">Submit</n-button>

			<h2>Bulk Upload</h2>
			<n-alert type="warning" v-if="mockBulkErrorMsg">{{
					mockBulkErrorMsg
				}}
			</n-alert>
			<n-upload
				ref="mockBulkUploadElem"
				@change="uploadChangedEvent"
				:max="1"
				class="mt-10">
				<n-upload-dragger>
			...
			<n-slider
				...
				:step="100"
				:format-tooltip="(val) => `${val / 1000} Kbps`" />
			<n-input-number
				class="mt-10"
				min="0"
				v-model:value="mockBulkDataSliderValue"
			...
			<n-alert
				v-if="mockBulkDataSliderValue > 19200"
				class="mt-10"
				title="Impossible Bitrate"
				type="info">
				<p>
					In real-world scenarios, bitrate will never exceed 19.2 Kbps
					(i.e., the baud rate).
				</p>
			</n-alert>
			...
			<n-progress
				...
				:percentage="
					mockBulkData
						? (Math.round(
								(mockBulkUploadProgress /
									mockBulkData.byteLength) *
									1000
						  ) /
								1000) *
						  100
...
const sports = ref<{
	[key: string]: { [key: number]: PacketDefinition };
} | null>(null);
...
const availablePortsOptions = computed<{ label: string; value: string }[]>(
	() => {
		...
		if (
			...
			replicants.sync.availablePorts.value.indexOf(
				replicants.sync.selectedPort.value
			) < 0
		) {
			arr.push({
				label: replicants.sync.selectedPort.value + " (Disconnected)",
				value: replicants.sync.selectedPort.value
			});
		}
...
const selectedMockPacketInfo = computed<PacketDefinition | null>(() => {
	...
	if (
		...
		!selectedPacket ||
		// @ts-ignore - FIXME Not sure why this is producing a TypeScript error.
		!sports.value[selectedSport][selectedPacket]
	) {
		return null;
	}
	// @ts-ignore - FIXME Not sure why this is producing a TypeScript error.
...
const mockChannel = new MessageComposable(
	"mock",
	"glimpse-graphics.daktronics"
);
...
function confirmShowAdvanced(): void {
	if (
		showAdvancedSettings.value &&
		!confirm(
			"Do not modify these settings unless you know what you are doing. Are you sure you want to continue?"
		)
	) {
...
async function mockBulkButtonClicked(): Promise<void> {
	...
	for (
		mockBulkUploadProgress.value = 0;
		mockBulkUploadProgress.value < (mockBulkData.value?.length || 0);
		mockBulkUploadProgress.value++
	) {
		...
		if (
			!mockBulkData.value ||
			mockBulkData.value.at(mockBulkUploadProgress.value) === undefined
		) {
			continue;
		}
		bytes.push(<number>mockBulkData.value.at(mockBulkUploadProgress.value));
		if (
			mockBulkUploadProgress.value - lastSentByteNumber >=
			selectedBitsPerSecond / 10
		) {
...
async function submitMockPacket(): Promise<void> {
	...
	const payload = [
		...
		0x30,
		0x30,
		0x01, // Padding? followed by ID start byte?
		0x30,
		0x30,
		0x34,
		0x32,
	...
	const checksumAscii = new TextEncoder().encode(
 ...
 */
function uploadChangedEvent(uploadEvent: {
	...
	fr.addEventListener("loadend", (readEvent) => {
		...
		const fileContents = new Uint8Array(
			<ArrayBuffer>readEvent.target.result
		);
		// First byte is always 0x16 in a valid RTD feed.
		if (fileContents.at(0) !== 0x16) {
			mockBulkErrorMsg.value =
				"That file doesn't appear to be a valid Daktronics RTD feed.";
			if (mockBulkUploadElem.value) {
				mockBulkUploadElem.value.clear();
			}
			mockBulkData.value = null;
