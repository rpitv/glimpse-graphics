...
//   port.
const mockMessageChannel = new MessageComposable(
	"mock",
...
new MessageComposable("sports-request", "glimpse-graphics.daktronics").listen(
	(argument, ack) => {
		ack(null, sports);
	}
);
...
async function openSerialPort(
	...
	port.on("data", async (data) => {
		bitrateCalculationCache.push({
			timestamp: Date.now(),
			byteCount: data.length
		});
		await new MessageComposable("data", "glimpse-graphics.daktronics").send(
			data
		);
	});
...
setInterval(() => {
	...
	bitrateCalculationCache = bitrateCalculationCache.filter(
		(c) => c.timestamp > Date.now() - 5000
	);
	// Calculate total number of bytes received within our 5-second window.
	const byteCount = bitrateCalculationCache.reduce(
		(a, b) => a + b.byteCount,
		0
	);
	...
	const oldestTimestamp = bitrateCalculationCache.reduce(
	...
	replicants.sync.status.value.bitrate =
		Math.floor((byteCount / ((Date.now() - oldestTimestamp) / 1000)) * 8) ||
		0;
}, 1000);
...
setInterval(async () => {
	replicants.sync.availablePorts.value = (await SerialPort.list()).map(
		(p) => p.path
	);
}, 1000);
