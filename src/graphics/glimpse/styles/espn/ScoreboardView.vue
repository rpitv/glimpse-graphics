<template>
	<div
		:class="
			'scoreboard ' +
			(replicants.scoreboard.visible.value ? '' : 'hidden')
		">
		<div class="team2-section">
			<TeamView
				class="bordered"
				v-if="teamTwo.enabled.value"
				:team-id="1" />

			<p
				v-if="replicants.announcements.team2.value.length > 0"
				class="announcement-section team2">
				{{
					computedMessage(replicants.announcements.team2.value[0])
						.value
				}}
			</p>
		</div>
		...
		<div class="team1-section">
			<TeamView
				class="bordered no-left-border"
				v-if="teamOne.enabled.value"
				:team-id="0" />

			<p
				v-if="replicants.announcements.team1.value.length > 0"
				class="announcement-section team1">
				{{
					computedMessage(replicants.announcements.team1.value[0])
						.value
				}}
			</p>
		</div>
		...
		<div class="bordered no-left-border time-section">
			<p
				v-if="replicants.gameSettings.periods.enabled.value"
				class="period-section">
				{{ formattedPeriod }}
			</p>
			<hr />
			<p
				v-if="replicants.gameSettings.clock.enabled.value"
				class="clock-section">
				{{ formattedClockTime }}
			</p>
			<p
				v-if="replicants.announcements.global.value.length > 0"
				class="announcement-section global">
				{{
					computedMessage(replicants.announcements.global.value[0])
						.value
				}}
			</p>
		</div>
...
const formattedPeriod = computed<string>(() => {
	if (period.value > replicants.gameSettings.periods.count.value) {
		const overtimePeriod =
			period.value - replicants.gameSettings.periods.count.value;
		if (overtimePeriod === 1) {
			return "OT";
		} else {
...
const team1TextColor = computed(() =>
	pickTextColorBasedOnBgColorSimple(team1Color.value, "#ffffff", "#000000")
);
const team2TextColor = computed(() =>
	pickTextColorBasedOnBgColorSimple(team2Color.value, "#ffffff", "#000000")
);
